# Types (action constants)

Source of truth: `packages/state/src/helpers/types.ts`.

Same pattern as repo `docs/context/types.tsx`: string constants dispatched through reducers and `data-views`.

```ts
/** App domain */
export const GET_SERMONS = 'GET_SERMONS';
export const GET_SERMON = 'GET_SERMON';
export const GET_PLAYLISTS = 'GET_PLAYLISTS';
export const GET_PLAYLIST = 'GET_PLAYLIST';
export const GET_MINISTERS = 'GET_MINISTERS';
export const GET_MINISTER = 'GET_MINISTER';
export const GET_LISTENERS = 'GET_LISTENERS';
export const GET_LISTENER = 'GET_LISTENER';
export const GET_CREATORS = 'GET_CREATORS';
export const GET_CREATOR = 'GET_CREATOR';
export const GET_LIBRARY = 'GET_LIBRARY';
export const GET_DISCOVERY_HOME = 'GET_DISCOVERY_HOME';
export const GET_FEATURED_MINISTER = 'GET_FEATURED_MINISTER';
export const GET_SEARCH_RESULTS = 'GET_SEARCH_RESULTS';
export const GET_PLANS = 'GET_PLANS';
export const GET_PLAN = 'GET_PLAN';
export const GET_TRANSACTIONS = 'GET_TRANSACTIONS';
export const GET_TRANSACTION = 'GET_TRANSACTION';

/** User domain */
export const GET_USERS = 'GET_USERS';
export const GET_USER = 'GET_USER';
export const GET_PROFILE = 'GET_PROFILE';
export const GET_PREFERENCES = 'GET_PREFERENCES';
export const GET_SUBSCRIPTION = 'GET_SUBSCRIPTION';

export const SET_USER = 'SET_USER';
export const SET_USER_TYPE = 'SET_USER_TYPE';
export const SET_PROFILE = 'SET_PROFILE';
export const SET_PREFERENCES = 'SET_PREFERENCES';
export const SET_PERMISSIONS = 'SET_PERMISSIONS';
export const SET_SUBSCRIPTION = 'SET_SUBSCRIPTION';
export const SET_PLAN = 'SET_PLAN';

export const SET_LOADING = 'SET_LOADING';
export const UNSET_LOADING = 'UNSET_LOADING';
export const SET_TOAST = 'SET_TOAST';
export const SET_SIDEBAR = 'SET_SIDEBAR';
```

Public API: `export * from './helpers/types'` in `packages/state/src/index.ts`.
