...
## MCP requirement (pacepard-ui-agent vs Figma hosted)

- **pacepard-ui-agent** (this repo’s MCP) does **not** register Code Connect tools such as `get_code_connect_suggestions`, `send_code_connect_mappings`, or `get_code_connect_map`.
- If the user **only** has pacepard-ui-agent: follow **Template-first workflow** below — author `.figma.ts` by hand or from design inspection via **`get_design_context`** / **`read_ui_power_steering`**, then use the [Code Connect CLI](https://developers.figma.com/docs/code-connect/) per Figma docs. Do not claim hosted MCP tools exist.
- If the user has **Figma hosted MCP** enabled: you may use the hosted Code Connect MCP tools where available and entitled.

## Prerequisites

- **For hosted MCP path:** Figma hosted MCP connected; verify tools like `get_code_connect_suggestions` exist before using steps that call them.
