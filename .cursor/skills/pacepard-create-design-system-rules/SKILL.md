---
name: pacepard-create-design-system-rules
description: Generates custom design system rules for the user's codebase. Use when user says "create design system rules", "generate rules for my project", "set up design rules", "customize design system guidelines", or wants project-specific Figma-to-code conventions. Requires pacepard-ui-agent MCP (tool create_design_system_rules).
...
- **pacepard-ui-agent** MCP must be connected (`bun socket`, plugin, `join_channel`). See [`skills/pacepard-ui-agent/SKILL.md`](../pacepard-ui-agent/SKILL.md).
...
Call the **pacepard-ui-agent** MCP tool **`create_design_system_rules`** to get the foundational prompt and template.
...
#### pacepard-ui-agent integration rules
...
1. join_channel, then get_design_context for the exact node(s) (optional fileKey per COMPATIBILITY)
2. If too large or truncated: get_metadata, then get_design_context on specific child ids
3. get_screenshot for visual reference
4. Assets: use export_node_as_image / project asset pipeline — pacepard-ui-agent has no hosted Figma asset CDN
5. Translate structured context into this project's conventions, styles, and framework
6. Validate against screenshot and design context before marking complete

### Implementation rules

- Treat design context as representation of design and behavior, not as final code style
...
```markdown
## Asset handling (pacepard-ui-agent)

- pacepard-ui-agent does not mirror Figma hosted MCP asset URLs
- Use get_screenshot and export_node_as_image where needed; place files under `[ASSET_DIRECTORY]` per project conventions
- Do not add icon packages for assets that should come from design exports
- Do not use placeholders when real exports exist
```
...
- IMPORTANT: Use pacepard-ui-agent export/screenshot flows for assets (no hosted MCP asset CDN)
...
- [Figma hosted MCP documentation](https://developers.figma.com/docs/figma-mcp-server/) (optional; for tools not on pacepard-ui-agent)
