> Part of the [pacepard-generate-library skill](../SKILL.md).

# Naming Conventions Reference

This reference documents every naming convention used in the pacepard-generate-library workflow. Cover all naming decisions in order: variables, components, pages, variants, styles, separators, status indicators. The last section explains when to match an existing file's conventions vs. using the defaults here.
