...
function validateCreation(checks) {
    ...
    for (const check of checks) {
        ...
        // Type check
        if (
            check.expectedType !== undefined &&
            node.type !== check.expectedType
        ) {
            reasons.push(
                `type is "${node.type}", expected "${check.expectedType}"`,
            );
        }

        // Name check
        if (
            check.expectedName !== undefined &&
            node.name !== check.expectedName
        ) {
            reasons.push(
                `name is "${node.name}", expected "${check.expectedName}"`,
            );
        }
