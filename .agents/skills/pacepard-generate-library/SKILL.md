---
name: pacepard-generate-library
description: 'Build or update a professional-grade design system in Figma from a codebase. Use when the user wants to create variables/tokens, build component libraries, set up theming (light/dark modes), document foundations, or reconcile gaps between code and Figma. This skill teaches WHAT to build and in WHAT ORDER — it complements the pacepard-use skill which teaches HOW to call the Plugin API. Both skills should be loaded together.'
disable-model-invocation: false
---

## MCP requirement

- **Canvas writes** in this skill are **`use_figma`** scripts — they require **Figma hosted MCP**, **not** pacepard-ui-agent alone.
- **pacepard-ui-agent** can still be used in parallel for **reads** (`get_metadata`, `get_screenshot`, `get_design_context`) on the same open file — see [pacepard-ui-agent-validation.md](../pacepard-use/references/pacepard-ui-agent-validation.md).

# Design System Builder — hosted `use_figma` orchestration
...
**Prerequisites**: The **[pacepard-use](../pacepard-use/SKILL.md)** skill MUST also be loaded for every `use_figma` call. It provides Plugin API syntax rules (return pattern, page reset, ID return, font loading, color range). This skill provides design system domain knowledge and workflow orchestration.

**Always pass `skillNames: "pacepard-generate-library"` when calling `use_figma` as part of this skill.** This is a logging parameter — it does not affect execution.
...
> "I'm continuing a design system build. Run ID: {RUN_ID}. Load the pacepard-generate-library skill and resume from the last completed step."
