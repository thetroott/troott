...
## MCP requirement (read before any work)

- **`use_figma`** exists only on **Figma hosted MCP** (e.g. `mcp.figma.com`), **not** on **pacepard-ui-agent** in this repository.
- **pacepard-ui-agent** provides discrete tools (`create_frame`, `get_metadata`, `get_design_context`, …). It cannot run arbitrary Plugin API scripts.
- **Validation reads** (`get_metadata`, `get_screenshot`, `get_design_context`) can use **pacepard-ui-agent** when the same file is open in Figma Desktop with the relay + plugin. See [references/pacepard-ui-agent-validation.md](references/pacepard-ui-agent-validation.md).
- For **implement code from Figma** without `use_figma`, use **[pacepard-implement-design](../pacepard-implement-design/SKILL.md)** and **[pacepard-ui-agent](../pacepard-ui-agent/SKILL.md)**.
...
**Always pass `skillNames: "pacepard-use"` when calling `use_figma`.** (Legacy logs may say `figma-use` — same skill folder.)

**If the task involves building or updating a full page, screen, or multi-section layout in Figma from code**, also load [pacepard-generate-design](../pacepard-generate-design/SKILL.md). It provides the workflow for discovering design system components via `search_design_system`, importing them, and assembling screens incrementally. Both skills work together: this one for the API rules, that one for the screen-building workflow.
