---
name: pacepard-generate-design
description: "Use this skill alongside pacepard-use when the task involves translating an application page, view, or multi-section layout into Figma. Triggers: 'write to Figma', 'create in Figma from code', 'push page to Figma', 'take this app/page and build it in Figma', 'create a screen', 'build a landing page in Figma', 'update the Figma screen to match code'. This is the preferred workflow skill whenever the user wants to build or update a full page, screen, or view in Figma from code or a description. Discovers design system components, variables, and styles via search_design_system, imports them, and assembles screens incrementally section-by-section using design system tokens instead of hardcoded values."
disable-model-invocation: false
---

## MCP requirement

- **Screen building** here uses **`use_figma`** — requires **Figma hosted MCP**, not pacepard-ui-agent alone.
- **pacepard-ui-agent** can validate structure/screenshots on the same open file — see [pacepard-ui-agent-validation.md](../pacepard-use/references/pacepard-ui-agent-validation.md).
...
**MANDATORY**: You MUST also load [pacepard-use](../pacepard-use/SKILL.md) before any `use_figma` call. That skill contains critical rules (color ranges, font loading, etc.) that apply to every script you write.

**Always pass `skillNames: "pacepard-generate-design"` when calling `use_figma` as part of this skill.** This is a logging parameter — it does not affect execution.

## Skill Boundaries

- Use this skill when the deliverable is a **Figma screen** (new or updated) composed of design system component instances.
- If the user wants to generate **code from a Figma design**, switch to [pacepard-implement-design](../pacepard-implement-design/SKILL.md).
- If the user wants to create **new reusable components or variants**, use [pacepard-use](../pacepard-use/SKILL.md) directly.
- If the user wants to write **Code Connect mappings**, switch to [pacepard-code-connect](../pacepard-code-connect/SKILL.md).

## Prerequisites

- **Figma hosted MCP** with `use_figma` must be connected for writes; **pacepard-ui-agent** optional for validation reads
...
See [variable-patterns.md](../pacepard-use/references/variable-patterns.md) for binding patterns.
...
frame
    ...
    .forEach((node) => {
        ...
        if ('effectStyleId' in node && node.effectStyleId) {
            ...
            if (s)
                styles.effect.set(s.id, { name: s.name, id: s.id, key: s.key });
        }
    });
return {
...
See [text-style-patterns.md](../pacepard-use/references/text-style-patterns.md) and [effect-style-patterns.md](../pacepard-use/references/effect-style-patterns.md) for details.
...
For detailed API patterns and gotchas, load these from the [pacepard-use](../pacepard-use/SKILL.md) references as needed:

- [component-patterns.md](../pacepard-use/references/component-patterns.md) — importing by key, finding variants, setProperties, text overrides, working with instances
- [variable-patterns.md](../pacepard-use/references/variable-patterns.md) — creating/binding variables, importing library variables, scopes, aliasing, discovering existing variables
- [text-style-patterns.md](../pacepard-use/references/text-style-patterns.md) — creating/applying text styles, importing library text styles, type ramps
- [effect-style-patterns.md](../pacepard-use/references/effect-style-patterns.md) — creating/applying effect styles (shadows), importing library effect styles
- [gotchas.md](../pacepard-use/references/gotchas.md) — layout pitfalls (HUG/FILL interactions, counterAxisAlignItems, sizing order), paint/color issues, page context resets

## Error Recovery

Follow the error recovery process from [pacepard-use](../pacepard-use/SKILL.md#6-error-recovery--self-correction):
