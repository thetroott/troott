...
class SermonRepository {
  ...
  public async findByTopic(
    ...
    const sermons = await this.model.find({ topic })
      .sort(options.sort || "-createdAt")
      .skip(options.skip || 0)
      .limit(options.limit || 25)
      .populate(options.populate || "");
  ...
  public async getSermonsByMinister(
    ...
    const sermons = await this.model.find({ minister: ministerId })
      .sort(options.sort || "-createdAt")
      .skip(options.skip || 0)
      .limit(options.limit || 25)
      .populate(options.populate || "");
  ...
  public async findRecentlyPlayedByUser(
    ...
    if (options.populate) {
      ...
      sermons = await this.model.populate(
        sermons.map((s) => s.doc),
        [{ path: "minister" }, { path: "series" }, { path: "category" }]
      );
    }
  ...
  public async findFavoriteMinistersSermonsRandom(
    ...
    try {
      ...
      if (!sermons || sermons.length === 0) {
        ...
        result.data = sermons;
        result.message = "Favorite ministers sermons retrieved successfully";
      }
    } catch (error: any) {
      result = {
        error: true,
  ...
  public async getTrendingSermons(): Promise<IResult> {
    ...
    const sermons = await this.model.find({})
      .sort({ totalPlay: -1 })
      .limit(10)
      .lean();
    result.data = sermons;
