import { IFile, IResult, ISermonDoc } from "../utils/interfaces.util";
...
class SermonService {
  ...
  public async handleUploadSermon(data: IFile): Promise<IResult> {
    ...
    const { stream, metadataStream, info, mimeType, size, fileType, uploadId } =
      data;
  ...
  public async handlePublishSermon(data: PublishSermonDTO): Promise<IResult> {
    ...
    try {
      ...
      const findSermon = await sermonRepository.findByUploadId(data.sermon);
      ...
      const publishSermon: ISermonDoc | null = await Sermon.findByIdAndUpdate(
        sermonId,
        {
          title,
          description,
