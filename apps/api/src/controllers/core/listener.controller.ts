...
/**
 * @name onboardTopics
 * @route POST /listener/onboarding/topics
 * @access Private
 */
export const onboardTopics: RequestHandler = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        const userId = (req as any).user?.id;
        if (!userId) return next(new ErrorResponse('Unauthorized', 401, []));

        const { topicIds }: OnboardTopicsDTO = req.body;

        if (!topicIds || !Array.isArray(topicIds) || topicIds.length === 0) {
            return next(
                new ErrorResponse(
                    'topicIds must be a non-empty array',
                    400,
                    [],
                ),
            );
        ...
        const result = await listenerService.onboardTopics(
            String(userId),
            topicIds,
        );
...
/**
 * @name onboardMinisters
 * @route POST /listener/onboarding/ministers
 * @access Private
 */
export const onboardMinisters: RequestHandler = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        const userId = (req as any).user?.id;
        if (!userId) return next(new ErrorResponse('Unauthorized', 401, []));

        const { ministerIds }: OnboardMinistersDTO = req.body;

        if (
            !ministerIds ||
            !Array.isArray(ministerIds) ||
            ministerIds.length === 0
        ) {
            return next(
                new ErrorResponse(
                    'ministerIds must be a non-empty array',
                    400,
                    [],
                ),
            );
        ...
        const result = await listenerService.onboardMinisters(
            String(userId),
            ministerIds,
        );
...
/**
 * @name skipOnboarding
 * @route POST /listener/onboarding/skip
 * @access Private
 */
export const skipOnboarding: RequestHandler = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        const userId = (req as any).user?.id;
        if (!userId) return next(new ErrorResponse('Unauthorized', 401, []));

        const result = await listenerService.skipOnboarding(String(userId));
