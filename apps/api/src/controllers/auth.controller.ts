...
export const activateUserAccount = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user.isActive) {
      return next(new ErrorResponse("Account is already activated", 401, []));
    }
    ...
    const responseData = await authMapper.mapActivatedUser(user, token.data?.token as any)
...
export const loginUser = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const responseData = await authMapper.mapUser(userExist, token.data as any)
...
export const forgotPassword = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user.passwordType === PasswordType.OAUTH) {
      return next(
        new ErrorResponse("This account was created via social login.", 403, []));
    }
...
export const changePassword = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user.passwordType === PasswordType.OAUTH) {
      return next(
        new ErrorResponse("You cannot change your password, account is managed by social login.", 403, []));
    }
...
export const socialAuthCallback = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) {
      return next(new ErrorResponse("Social authentication failed.", 401, []));
    }
