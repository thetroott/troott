 ...
 */
const audioMetadataProcessor = async (job: Job<IAudioMetadataJobDTO>, done: DoneCallback) => {
  ...
  try {
        ...
        if (!updateSermon) {
          ...
          logger.log({ data: errorMessage, label: "audio-metadata-processor", type: "error" });
