...
import { determineFileType, genFileName } from "../utils/helper.util";
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (contentType?.includes("multipart/form-data")) {
      ...
      stream.on("file", (fieldname, file, info: FileInfo) => {
        ...
        if (!acceptedMimeType.includes(mimeType as FileMimeType)) {
          file.resume();
          return next(
            new ErrorResponse(`File "${filename}" is not supported.`, 400, [])
          );
        }
    ...
    if (contentType?.includes("application/json")) {
      ...
      if (format !== FileFormat.BASE64) {
        return next(new ErrorResponse(`Invalid json format: ${format}`, 400, []));
      }

      if (!base64 || !type) {
        return next(new ErrorResponse("Missing base64 or type", 400, []));
      }
      ...
      if (!acceptedMimeType.includes(mimeType as FileMimeType)) {
        return next(
          new ErrorResponse(`Unsupported file type: ${mimeType}`, 400, [])
        );
      }
