...
export default function checkPermission(
  ...
  return asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (options?.ownerResolver) {
      ...
      resourceOwnerId = (req.params && req.params[options.ownerParam]) || (req.body && req.body[options.ownerParam]) || (req.query && (req.query as any)[options.ownerParam]) || null;
    }

    const permsToCheck = Array.isArray(perm) ? perm : [perm];

    for (const p of permsToCheck) {
      const ok = await PermissionService.hasPermission(user, p as any, { resourceOwnerId, checkOwnership });
      if (ok) return next();
    }
