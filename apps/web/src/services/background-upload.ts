...
class BackgroundUploadService {
    ...
    startUpload(file: File): string {
        ...
        const estimatedTimeInSeconds = Math.max(
            10,
            Math.min(60, fileSizeInMB * 2),
        );
    ...
    offComplete(callback: UploadCompleteCallback): void {
        this.listeners.complete = this.listeners.complete.filter(
            (fn) => fn !== callback,
        );
    }
    ...
    offCancelled(callback: UploadCancelledCallback): void {
        this.listeners.cancelled = this.listeners.cancelled.filter(
            (fn) => fn !== callback,
        );
    }
