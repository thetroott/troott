...
const nestedRnInsideEdgeRe = /[/\\]react-native-is-edge-to-edge[/\\]node_modules[/\\]react-native(?=[/\\]|$)/;
...
function attachMonorepoResolver(config) {
  ...
  config.resolver.resolveRequest = (context, moduleName, platform) => {
    ...
    if (
      ...
      (typeof moduleName === "string" && moduleName.startsWith("react-native/"))
    ) {
      const r = tryResolveFromProject(moduleName);
      if (r) {
        return r;
    ...
    if (
      ...
      (typeof moduleName === "string" && moduleName.startsWith("react-native-svg/"))
    ) {
      const r = tryResolveFromProject(moduleName);
      if (r) {
        return r;
    ...
    if (
      ...
      (typeof moduleName === "string" && moduleName.startsWith("react-native-reanimated/"))
    ) {
      const r = tryResolveFromProject(moduleName);
      if (r) {
        return r;
    ...
    if (
      ...
      (typeof moduleName === "string" && moduleName.startsWith("react-native-worklets/"))
    ) {
      const r = tryResolveFromProject(moduleName);
      if (r) {
        return r;
