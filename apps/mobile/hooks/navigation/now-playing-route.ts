...
function getDeepestFocusedRouteName(state: NavigationState | undefined): string | undefined {
	if (!state?.routes?.length) return undefined;
	const route = state.routes[state.index ?? 0];
	if (route.state) {
		const nested = getDeepestFocusedRouteName(route.state as NavigationState);
...
function computeIsNowPlayingFocused(
	...
	return isNowPlayingRouteName(getDeepestFocusedRouteName(state as NavigationState));
}

/**
 * True when the root stack is showing the now-playing modal (`track` / `player`), regardless of tab pathname.
 *
...
export function useIsNowPlayingStackRouteFocused(): boolean {
	...
	useLayoutEffect(() => {
		...
		const syncFromState = (root: NavigationState | PartialState<NavigationState> | undefined) => {
