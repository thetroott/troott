...
export const useTrackStore = create<IPlayerState>((set, get) => ({
  ...
  togglePlayPause: () =>
    set({ trackPlaying: !get().trackPlaying }),
  ...
  setCurrentTime: (time) => {
    ...
    set({ currentTime: clamped, progress: duration ? clamped / duration : 0 });
  },
  duration: 0,
  setDuration: (duration) => {
    const { currentTime } = get();
    const clampedTime = Math.max(0, Math.min(currentTime, duration || Number.MAX_SAFE_INTEGER));
    set({
      duration,
