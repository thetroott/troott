...
const FavoriteMinisters = () => {
  ...
  return (
    <KeyboardAvoidingView className="flex-1" behavior="padding">
      ...
      <View className="absolute bottom-0 left-0 right-0 gap-5 justify-end bg-black/90 p-6">
        <Button
...
// Allowed exception: runtime dimension (screen width) for grid card size.
...
function PastorCard({ name, image, selected, onPress }: PastorCardProps) {
  return (
    ...
    >
      ...
      <Text className="text-center text-neutral-100">{name}</Text>
    </Pressable>
  );
}
