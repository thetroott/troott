 ...
 */
export async function findPlayQueueIndexStart(playQueue: SermonTrackDTO[], currentIndex: number) {
	if (isEmpty(playQueue)) return 0
 ...
 */
export function fetchManuallyQueuedTracks(queue: SermonTrackDTO[]): SermonTrackDTO[] {
	return queue.filter(
		(track) =>
