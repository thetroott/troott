...
function resolveBundledMediaUrl(url: string | number | null | undefined): string | number {
...
function guessMimeType(url: string | number | null | undefined): string | undefined {
...
function pickPlaybackUrl(
	...
	const secondary = shareable === "" || shareable == null ? undefined : shareable
...
export function mapDtoToTrack(
	...
	let resolvedUrl: string | number =
		typeof playbackUrl === "number"
			? playbackUrl
			: resolveBundledMediaUrl(typeof playbackUrl === "string" ? playbackUrl : "")

	if (typeof resolvedUrl === "string" && resolvedUrl.length > 0 && isProbablyImageFileUrl(resolvedUrl)) {
		console.warn(
	...
	const resolvedArtwork =
		typeof artworkUrl === "number" ? resolveBundledMediaUrl(artworkUrl) : artworkUrl

	const mimeType =
		typeof resolvedUrl === "string" ? guessMimeType(resolvedUrl) : undefined
	...
	return {
		...
		albumTitle: item.seriesTitle ?? (Array.isArray(item.series) ? item.series[0] : undefined),
		album: item.seriesTitle ?? (Array.isArray(item.series) ? item.series[0] : undefined),
		...
		item: {
			...
			title: item.title,
			image:
				item.image != null && (typeof item.image === "string" || typeof item.image === "number")
					? item.image
					: null,
