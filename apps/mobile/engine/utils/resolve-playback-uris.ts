...
function isLikelyImageBundledAsset(uri: string, assetType: string | null | undefined): boolean {
	...
	return Promise.all(
		tracks.map(async (track) => {
			...
			try {
				...
				const mime = track.mimeType ?? mimeFromAssetExtension(asset.type)
				return { ...track, url: uri, ...(mime ? { mimeType: mime } : {}) }
			} catch (e) {
				console.warn("[resolvePlaybackUris] bundled asset failed", track.mediaId, e)
				return track
			}
		}),
	)
