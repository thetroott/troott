	...
	try {
		...
		const options = {
			...
			progress: (data: any) => {
				...
				setDownloadProgress((prev: SermonDownloadProgress) => ({
					...prev,
					[url]: { progress: percent, name: fileName, sermonName: sermonName },
				}))
			},
			background: true,
			progressDivider: 1,
...
function mediaUrlToDownloadString(
	url: SermonTrackDTO["url"],
): string {
...
export const saveAudio = async (
	...
	const existingRaw = getOfflineKV().getString(MMKV_OFFLINE_MODE_KEYS.AUDIO_CACHE)
	...
	getOfflineKV().set(MMKV_OFFLINE_MODE_KEYS.AUDIO_CACHE, JSON.stringify(existingArray))
...
const setAudioCache = (downloads: SermonDownload[]) => {
	getOfflineKV().set(MMKV_OFFLINE_MODE_KEYS.AUDIO_CACHE, JSON.stringify(downloads))
}
...
const deleteDownloadAssets = async (download: SermonDownload): Promise<number> => {
	let freedBytes = 0
