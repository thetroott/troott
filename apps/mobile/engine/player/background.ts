...
export function attachEnginePlaybackListeners(): void {
	...
	TrackPlayer.addEventListener(Event.PlaybackError, ({ code, message }) => {
		try {
			const active = TrackPlayer.getActiveMediaItem() as { url?: unknown } | null | undefined
			const url = active?.url
			console.error("[TrackPlayer] PlaybackError", code, message, "activeUrl:", url)
		} catch {
			console.error("[TrackPlayer] PlaybackError", code, message)
		}
	})
}
