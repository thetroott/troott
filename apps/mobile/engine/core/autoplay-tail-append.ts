...
import { getAutoplayCatalogAsSermonItems, mergeAutoplayTail } from "@/engine/core/autoplay-tail"
...
export async function appendAutoplayTailToPlayerQueueIfNeeded(): Promise<void> {
	...
	const tail = withMinister.filter((t) => t.id && !excludeIds.has(String(t.id))).slice(0, MAX_TAIL_APPEND)
