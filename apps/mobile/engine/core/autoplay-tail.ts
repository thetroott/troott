...
export function getAutoplayCatalogAsSermonItems(): SermonItemDTO[] {
	...
	if (cached && cached.length > 0) {
		return cached.map((r) => catalogRowToSermonItem({ ...r, id: r.id ?? null }))
	}
	return mockTracks.map((r) => catalogRowToSermonItem({ ...r, id: r.id ?? null }))
}
...
export function mergeAutoplayTail(
	...
	const candidates = tailSource.filter((t) => t.id && !seen.has(String(t.id)))
