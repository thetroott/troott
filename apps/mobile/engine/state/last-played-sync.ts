...
import { usePlayerQueueStore, type LastPlayedSummary } from "@/engine/state/player-queue-store"
...
function trackToSummary(track: SermonTrackDTO, positionSec: number): LastPlayedSummary {
...
export function mergeLastPlayedPosition(positionSec: number, durationSec: number): void {
...
export function recordLastPlayedFromTrack(track: SermonTrackDTO | undefined): void {
	if (!track) return
...
export function lastPlayedToSermonItemDto(lp: LastPlayedSummary): SermonItemDTO {
	return {
		id: lp.sermonId,
		title: lp.title,
		minister: lp.artist,
