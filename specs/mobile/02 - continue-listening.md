...
1. **Repeat queue (already in the engine)**
  You already have `RepeatMode.Queue`. When the user has "Repeat queue" on, the queue loops forever. That *is* continuous listening. Ensure the repeat-mode control is visible and that restoring `repeatMode` on startup (which `Initialize()` does) is enough. No backend needed.
...
- **Persisted queue state** – `usePlayerQueueStore` (MMKV) persists: `queue`, `currentIndex`, `currentTrack`, `queueRef`, `repeatMode`, `shuffled`, `unShuffledQueue`. So we can know *which* track and *which* queue after restart.
